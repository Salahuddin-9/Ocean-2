...
export default function EvidenceVault({ token, currentUser, onClose }: EvidenceVaultProps) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const d = await api('/api/evidence/entries', 'GET');
  ...
  const saveEntry = async () => {
    ...
    try {
      ...
      await api('/api/evidence/entries', 'POST', { title, kind: 'note', iv, ciphertext: ct });
  ...
  const openEntry = async (id: string) => {
    setBusy(true);
    try {
      const d = await api(`/api/evidence/entries/${id}`, 'GET');
  ...
  const removeEntry = async (id: string) => {
    try {
      await api(`/api/evidence/entries/${id}`, 'DELETE');
      toast('Entry deleted.');
      setViewing(null);
      await load();
